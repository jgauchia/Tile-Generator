...
If a `ROUTE/` directory with profile subfolders (`CAR/`, `BIKE/`, `WALK/`) exists alongside the nav dir (generated by `route_generator`), the viewer supports interactive routing. The active profile defaults to **WALK** and is switchable in-app:
