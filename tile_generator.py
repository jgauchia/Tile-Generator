...
def get_layer_from_tags(tags: Dict) -> int:
    ...
    if 'landuse' in tags and tags['landuse'] in ['forest', 'farmland', 'meadow', 'grass', 
                                                   'orchard', 'vineyard', 'residential', 
                                                   'commercial', 'retail', 'industrial']:
        return 1
    if 'natural' in tags and tags['natural'] in ['wood', 'forest', 'scrub', 'heath', 
                                                   'grassland', 'beach', 'sand', 'wetland']:
        return 1
    if 'leisure' in tags and tags['leisure'] in ['park', 'pitch', 'golf_course', 
                                                   'stadium', 'sports_centre', 'playground']:
        return 1
...
def compress_goql_queries(config: Dict) -> str:
    METADATA_KEYS = {
        'tile_size', 'viewport_size', 'toolbar_width', 'statusbar_height',
        'max_cache_size', 'thread_pool_size', 'background_colors',
        'log_level', 'config_file', 'fps_limit', 'fill_polygons'
    }
    
    LINEAR_TAGS = {'highway', 'railway', 'waterway'}
    
    LINEAR_FEATURE_VALUES = {
        'natural': {'coastline', 'tree_row'},
    ...
    for key in config:
        if key in METADATA_KEYS or not isinstance(config[key], dict):
            continue
        
        if '=' in key:
            tag, value = key.split('=', 1)
            
            is_linear = False
            if tag in LINEAR_FEATURE_VALUES and value in LINEAR_FEATURE_VALUES[tag]:
                is_linear = True
            elif tag in LINEAR_TAGS and tag not in AREA_FEATURE_VALUES:
                is_linear = True
            
            is_area = False
            if tag in AREA_FEATURE_VALUES and value in AREA_FEATURE_VALUES[tag]:
                is_area = True
            
            if is_linear and not is_area:
                way_queries.append(tag + "=" + value)
            else:
                area_queries.append(tag + "=" + value)
        else:
            area_queries.append(key)
    
    def group_by_tag(queries):
        grouped = {}
        single = []
        for q in queries:
            if '=' in q:
    ...
    way_grouped, _ = group_by_tag(way_queries)
    for tag, vals in sorted(way_grouped.items()):
        if len(vals) == 1:
            query_parts.append("w[" + tag + "=" + vals[0] + "]")
        else:
            pattern = '|'.join(vals)
            query_parts.append("w[" + tag + "~'^(" + pattern + ")$']")
    
    area_grouped, area_single = group_by_tag(area_queries)
    for tag, vals in sorted(area_grouped.items()):
        if len(vals) == 1:
            query_parts.append("nwa[" + tag + "=" + vals[0] + "]")
        else:
            pattern = '|'.join(vals)
            query_parts.append("nwa[" + tag + "~'^(" + pattern + ")$']")
    
    for tag in sorted(area_single):
        query_parts.append("nwa[" + tag + "]")
    
    final_query = ", ".join(query_parts) if query_parts else "*"
    
    logger.info("Generated GoQL query: " + final_query[:200] + ("..." if len(final_query) > 200 else ""))
...
def generate_tiles(gol_file: str, output_dir: str, config_file: str, zoom_levels: List[int], max_file_size: int = 65536):
    ...
    for zoom in zoom_levels:
        ...
        if tiles_data:
            ...
            tile_jobs = [(tx, ty, entries, zoom, output_dir, max_file_size) 
                        for (tx, ty), entries in tiles_data.items()]
    ...
    return total_tiles_written

def main():
    parser = argparse.ArgumentParser(
        description='Generate map tiles from GoL file using features configuration',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate tiles for zoom levels 6-17
  ./tile_generator.py map.gol output_dir features.json --zoom 6-17
  
  # Generate only zoom 12 with 256KB max tile size
  ./tile_generator.py map.gol output_dir features.json --zoom 12 --max-file-size 256
    ...
    parser.add_argument("--zoom", default="6-17", 
                       help="Zoom level(s) to generate (e.g., '12' or '6-17')")
    parser.add_argument("--max-file-size", type=int, default=128, 
                       help="Maximum tile file size in KB (default: 128)")
        ...
        if os.path.exists(zoom_dir):
            for x_dir in os.listdir(zoom_dir):
                ...
                if os.path.isdir(x_path):
                    actual_count += len([f for f in os.listdir(x_path) if f.endswith('.bin')])
    
    logger.info("✓ Total tiles written: " + str(actual_count))
