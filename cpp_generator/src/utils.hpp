...
/**
 * @brief Standard Mercator latitude projection.
 * @param lat Latitude in degrees.
 * @return Projected Y.
 */
inline double lat_to_merc(double lat)
{
    double r = lat * M_PI / 180.0;
    r = std::max(-0.999 * M_PI / 2.0, std::min(0.999 * M_PI / 2.0, r));
 ...
 */
inline double lat_from_tile_y(int y, int z)
{
    double n = std::pow(2.0, z);
...
inline uint8_t meters_to_pixels(double width_meters, int zoom, double lat = 45.0)
{
    ...
    int pixels = static_cast<int>(width_meters / meters_per_pixel + 0.5);
