...
private:
    ...
    void process_zoom_level(std::vector<Feature> (&features_by_zoom)[18], int z)
    {
        ...
        while (next_tile < tiles.size())
        {
            ...
            if (zoom_bytes < 1024 * 1024) std::cout << std::fixed << std::setprecision(1) << (zoom_bytes / 1024.0) << " KB" << std::flush;
            else std::cout << std::fixed << std::setprecision(1) << (zoom_bytes / 1024.0 / 1024.0) << " MB" << std::flush;
        ...
        if (zoom_bytes < 1024 * 1024) std::cout << std::fixed << std::setprecision(2) << (zoom_bytes / 1024.0) << " KB";
        else std::cout << std::fixed << std::setprecision(2) << (zoom_bytes / 1024.0 / 1024.0) << " MB";
    ...
    void write_tile(int z, int x, int y, std::vector<Feature> (&features_by_zoom)[18], const std::vector<std::pair<int, size_t>>& indices, GEOSContextHandle_t local_handle, size_t& merged_out, uint64_t& bytes_out)
    {
        ...
        auto protect_esp32 = [&](GEOSGeometry* g) {
            ...
            while (pts > MAX_SAFE_POINTS && current_tol < (tolerance * 20)) 
            {
                ...
                if (temp) { if (current != g) GEOSGeom_destroy_r(local_handle, current); current = temp; pts = count_geos_points(local_handle, current); } else break;
            }
            return current;
        };
        ...
        if (out)
        {
            ...
            for (const auto& pf : final_features)
            {
                ...
                out.write((char*)fh, 13); out.write((char*)payload.data(), payload.size()); if (!ri.empty()) out.write((char*)ri.data(), ri.size());
                count++; if (count == 0xFFFF) break;
            }
            bytes_out = (uint64_t)out.tellp();
            out.seekp(4); out.write((char*)&count, 2);
