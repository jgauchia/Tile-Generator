 ...
 * @brief Optimized tile generation engine with mapped storage support.
 * @version 0.4.0
 * @date 2026-02
 */
...
public:
    ...
    /**
     * @brief Main entry point for processing all zoom levels.
     */
...
private:
    ...
    uint32_t count_geos_points(GEOSContextHandle_t handle, const GEOSGeometry* g)
    {
        ...
        if (type == GEOS_LINESTRING || type == GEOS_LINEARRING)
        {
            ...
            if (s)
                GEOSCoordSeq_getSize_r(handle, s, &count);
            return count;
        }
    ...
    void process_zoom_level(std::vector<size_t> (&features_by_zoom)[18], MappedStore& store, int z)
    {
        ...
        for (unsigned int t = 0; t < num_threads; ++t)
        {
            workers.emplace_back([&]() {
                ...
                while (true)
                {
                    ...
                    write_tile(z, coord.x, coord.y, store, offsets, handle, m, b);
                    merged_count += m;
                    zoom_bytes += b;
                }
        ...
        while (next_tile < tiles.size())
        {
            ...
            std::cout << "\r\33[2K  Zoom " << std::setw(2) << z << ": [" << std::string(progress * 20, '#') << std::string(20 - progress * 20, '-') << "] " << std::setw(3) << int(progress * 100.0) << "% | " << std::setw(8) << (size_t)next_tile << "/" << tiles.size() << " tiles | " << std::fixed << std::setprecision(1) << tps << " t/s | ";
     ...
     */
    void write_tile(int z, int x, int y, const MappedStore& store, const std::vector<size_t>& offsets, GEOSContextHandle_t local_handle, size_t& merged_out, uint64_t& bytes_out)
    {
        struct Style
        {
            uint16_t color;
        ...
        GEOSCoordSeq_setX_r(local_handle, cbox, 0, lmin - lm);
        GEOSCoordSeq_setY_r(local_handle, cbox, 0, tmin - tm);
        GEOSCoordSeq_setX_r(local_handle, cbox, 1, lmax + lm);
        GEOSCoordSeq_setY_r(local_handle, cbox, 1, tmin - tm);
        GEOSCoordSeq_setX_r(local_handle, cbox, 2, lmax + lm);
        GEOSCoordSeq_setY_r(local_handle, cbox, 2, tmax + tm);
        GEOSCoordSeq_setX_r(local_handle, cbox, 3, lmin - lm);
        GEOSCoordSeq_setY_r(local_handle, cbox, 3, tmax + tm);
        GEOSCoordSeq_setX_r(local_handle, cbox, 4, lmin - lm);
        GEOSCoordSeq_setY_r(local_handle, cbox, 4, tmin - tm);
        ...
        auto protect_esp32 = [&](GEOSGeometry* g) {
            if (!g || GEOSisEmpty_r(local_handle, g))
                return (GEOSGeometry*)nullptr;
            ...
            while (pts > MAX_SAFE_POINTS && current_tol < (tolerance * 20))
            {
                ...
                {
                    if (current != g)
                        GEOSGeom_destroy_r(local_handle, current);
                    current = temp;
                    pts = count_geos_points(local_handle, current);
                }
                else
                    break;
            }
        ...
        for (auto& [style, geos_polys] : poly_groups)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (!g || GEOSGeomTypeId_r(local_handle, g) != GEOS_POLYGON || GEOSisEmpty_r(local_handle, g))
                    continue;
                ...
                {
                    if (safe && safe != simplified)
                        GEOSGeom_destroy_r(local_handle, safe);
                ...
                for (int h = 0; h < nh; ++h)
                    process_ring(GEOSGetInteriorRingN_r(local_handle, safe, h));
                if (!pf.rings.empty())
        ...
        for (const auto& f : lines)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (!part || (GEOSGeomTypeId_r(local_handle, part) != GEOS_LINESTRING && GEOSGeomTypeId_r(local_handle, part) != GEOS_LINEARRING) || GEOSisEmpty_r(local_handle, part))
                    continue;
        ...
        if (out)
        {
            ...
            memcpy(th + 6, &b_lmin, 4);
            memcpy(th + 10, &b_tmin, 4);
            memcpy(th + 14, &b_lmax, 4);
            memcpy(th + 18, &b_tmax, 4);
            ...
            for (const auto& pf : final_features)
            {
                ...
                if (pf.type == GEOM_POLYGON)
                {
                    ...
                    ri.push_back(nr & 0xFF);
                    ri.push_back(nr >> 8);
                    uint16_t cur = 0;
                    for (const auto& r : pf.rings)
                    {
                        cur += (uint16_t)r.size();
                        ri.push_back(cur & 0xFF);
                        ri.push_back(cur >> 8);
                    }
                }
